...
impl SurfaceWrapper {
    ...
    fn resume(&mut self, context: &RenderDeviceContext, window: Arc<Window>, srgb: bool) {
    ...
    fn resize(&mut self, context: &RenderDeviceContext, size: PhysicalSize<u32>) {
    ...
    fn acquire(&mut self, context: &RenderDeviceContext) -> wgpu::SurfaceTexture {
        let surface = self.surface.as_ref().unwrap();
...
async fn start<E: SceneRender>(title: &str) {
    ...
    let context = RenderDeviceContext::init_async::<E>(&mut surface, window_loop.window.clone()).await;
